...
;(function(window) {
	...
	var support = {transitions : Modernizr.csstransitions},
		...
		// 'select your seats' control 开始进入选择的button
		...
		// the initial values for the room transform 进入room的默认视觉
		initTransform = {
			translateX : 0,
...
//zoom room 的设置
	...
	function initEvents() {
		...
		// select seats control click (intro button): show the room layout //开始入口事件
	...
	function selectSeat(planseat) {
		...
		if( classie.has(planseat, 'row__seat--selected') ) {
			...
			seatArr = $.grep(seatArr, function(value){
				return value != $(planseat).attr('data-tooltip');
			});
			return false;
		}
		// add selected class
		...
		seatArr.push($(planseat).attr('data-tooltip'));
		// the real seat
	...
	function zoomOutScreen(callback) { // 进room的zoom
